...
class OpenAIHandler:
    def __init__(
        ...
        self.model = model or os.getenv("OPENAI_ASSISTANT_MODEL", "gpt-4o")
...
Example Answer
{
  ...
  "Next_steps": [
    ...
    },
    {
      ...
      "Reasoning": "From the returned forecast I need to select the best day. e.g. avoid rain and wind. So I can create a new todo item with the task 'Walk the dog' and the selected day as the due date.",
    ...
    {
      "Subtask": "Return short output to user with my choice and basic reason. e.g. I have checked this week's forecast and <day&date> looks like the nicest day for a walk. I have added it to your todo list.",
...
"""
        ...
        formatted_now = f" the current time is {datetime.now().astimezone().strftime('%Y-%m-%dT%H:%M:%S%z')}"

        messages_system = [
            {
                "role": "system",
                "content": f"{system_prompt}. {formatted_now}. {location}. {user}\n{example_prompt}",
            }
        ]
        ...
        messages_first_plan[0]["content"] = (
            f"To answer the users query, this is the original plan:\n"
            f"{messages_first_plan[0]['content']}"
        )
        ...
        messages_system = [
            {
                "role": "system",
                "content": f"{self.system_role}. {formatted_now}. {location}. {user}\n{example_prompt}",
            }
        ]
        messages_user = [{"role": "user", "content": query}]
                ...
                messages_curr_plan = [
                    {
                        "role": "assistant",
                        "content": (
                            "To answer the users request, I have made the following plan. "
                            "It needs to be updated based on the results from the function and used "
                            "if I make another function call in the 'planning' field.\n"
                            f"Plan: '{response.get('content')}'"
                        ),
                    }
                ]
            else:
