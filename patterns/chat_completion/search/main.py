...
GPT_MODEL = os.getenv("OPENAI_ASSISTANT_MODEL", "gpt-4o")
...
def assistant(user_input, messages):
    messages.append(
        {"role": "user", "content": user_input})  # Append user input to the messages list with the role "user"
    chat_response = chat_completion_request(messages,
                                            tools=tools)  # Call the chat_completion_request function to get a chat completion response
    assistant_message = chat_response.choices[
        0].message  # Get the message from the first choice in the chat completion response
    ...
    if tool_calls:  # Check if there are any tool calls
        ...
        print(
            colored(f"Function call requested: {tool_function_name}", "cyan"))  # Print the name of the function called
        ...
        if tool_function_name == 'fetch_website_data':
            ...
            timelimit = eval(tool_calls[0].function.arguments)[
                'timelimit']  # Get the timelimit argument from the tool call
            max_results = eval(tool_calls[0].function.arguments)[
                'max_results']  # Get the max_results argument from the tool call
            results = web_search(query, region, timelimit,
                                 max_results)  # Call the web_search function with the query, region, timelimit, and max_results arguments
            ...
            max_results = eval(tool_calls[0].function.arguments)[
                'max_results']  # Get the max_results argument from the tool call
            results = news_search(query, region, timelimit,
                                  max_results)  # Call the news_search function with the query, region, timelimit, and max_results arguments
        elif tool_function_name == 'get_transcript':
            video_url = eval(tool_calls[0].function.arguments)[
                'video_url']  # Get the video_url argument from the tool call
            ...
            text_content = eval(tool_calls[0].function.arguments)[
                'text_content']  # Get the text_content argument from the tool call
            file_path = eval(tool_calls[0].function.arguments)[
                'file_path']  # Get the file_path argument from the tool call
            results = save_to_file(text_content,
                                   file_path)  # Call the save_to_file function with the text_content and file_path arguments
        else:
        ...
        response_to_function = model_response_with_function_call.choices[
            0].message  # Get the message from the first choice in the chat completion response
